...
export class PlatformServer {
  ...
  private async listSessions(): Promise<Array<{ id: string; startedAt: number; endedAt: number | null }>> {
  ...
  private async deleteSession(sessionId: string): Promise<{ deletedEvents: number; deletedSessions: number }> {
    if (!this.dbService) {
      throw new Error('Database service not initialized');
    }

    return this.dbService.getInstance().transaction(async () => {
      await this.dbService!.getInstance().prepare('DELETE FROM pending_actions WHERE session_id = ?').run(sessionId);
      await this.dbService!.getInstance().prepare('DELETE FROM interaction_contexts WHERE session_id = ?').run(sessionId);

      await this.dbService!.getInstance().prepare(`
      ...
      const deleteEvents = await this.dbService!.getInstance().prepare('DELETE FROM events WHERE session_id = ?').run(sessionId);
      const deleteSessionResult = await this.dbService!.getInstance().prepare('DELETE FROM sessions WHERE id = ?').run(sessionId);
  ...
  private async exportSession(sessionId: string, outputPath: string): Promise<void> {
    ...
    const rows = await this.dbService.getInstance().prepare(`
      SELECT
        id,
        session_id,
        type,
  ...
  private async debugRecentEvents(limit = 5): Promise<Array<{ id: string; type: string; sequence: number | null; sessionId: string }>> {
    if (!this.dbService) {
      throw new Error('Database service not initialized');
    }
  ...
  private async handleExtensionRoutes(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {

...
/**
 * Returns true if the selector targets a concrete interactive control.
 * These must never be moved to ignoredSteps.
 */
 ...
 *
 * Conservative token list. App-agnostic — no hardcoded app names.
 * Splits hyphenated class names into sub-tokens for correct matching.
...
function isBroadContainerSelector(selector) {
    // Extract all word-boundary tokens, then split by '-' to handle hyphenated names
    const tokenPattern = /[\w-]+/g;
    const rawTokens = (selector.match(tokenPattern) ?? []).map((t) => t.toLowerCase());

    if (rawTokens.length === 0) return false;

    // Expand hyphenated tokens: 'background-container' → ['background', 'container']
    ...
    // If the selector has a descendant combinator, it's scoped — not broad

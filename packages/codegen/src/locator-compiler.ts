 ...
 */
function compileRegex(regex: RegexLiteralSpec): string {
 ...
 */
function compileValue(val: string | RegexLiteralSpec): string {
 ...
 */
function compileOptions(options: PlaywrightLocatorOptions): string | null {
  const parts: string[] = [];

  const nameIsRegex = typeof options.name === 'object' && options.name !== null;
  const hasTextIsRegex =
    typeof options.hasText === 'object' && options.hasText !== null;
 ...
 */
function compileNode(node: PlaywrightLocatorNode): string {
  const args: string[] = [escapeString(node.value)];
 ...
 */
export function compilePlaywrightLocator(spec: PlaywrightLocatorSpec): string {

import type { BoundedFieldControlKind, BoundedFieldRelation, BoundedFieldSelectorSpec, CodegenStep, LabelContextRenderStatus, ResolverSnapshotSource, SelectorSpec, SnapshotSelectionProvenance } from '../../types';
...
export interface BoundedFieldProofResult {
    ...
    rejectReason: 'bounded-field-no-label' | 'bounded-field-duplicate-label' | 'bounded-field-target-binding-failed' | 'bounded-field-multiple-targets' | 'bounded-field-broad-container' | 'bounded-field-no-renderable-scope' | 'target_selector_ambiguous' | 'target_not_bound_to_field_container' | 'bounded-field-global-duplicate-label' | 'invalid-target-spec' | null;
}

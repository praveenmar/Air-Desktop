import type { BoundedFieldSelectorSpec, CodegenAssertion, CodegenSession, CodegenStep, FlatSelectorEngine, LabelContextSelectorSpec, TriggerContextSelectorSpec, LlmResponseFormat, ResolverMetadata, ResolverSnapshotSource, SelectorCategory, SelectorEvaluation, SelectorSpec, SelectorPriority } from '../types';
...
export interface SnapshotCache {
    ...
    getSource?: (nodeId: string, normalizedUrl?: string, controlSignature?: string) => ResolverSnapshotSource;
    selectForStep?: (step: CodegenStep, mode?: 'action' | 'outcome') => SnapshotSelectionResult;
...
export interface RawCandidate {
    selector: string;
    source: 'original' | 'id' | 'name' | 'testid' | 'data-cy' | 'data-qa' | 'aria' | 'placeholder' | 'href' | 'role+name' | 'class' | 'text' | 'parent-scope' | 'other' | 'path' | 'chained' | 'bounded-field' | 'label-context' | 'trigger-context';
...
export type SelectorFallbackProvider = (request: SelectorFallbackRequest) => Promise<LlmFallbackSuggestion[] | NormalizedLlmRetrySuggestion[] | unknown[]>;
...
export type ResolverRejectReason = 'href_mismatch' | 'control_family_mismatch' | 'text_mismatch' | 'snapshot_target_missing';
export type ControlFamily = 'text-input' | 'password-input' | 'submit-button' | 'nav-link' | 'menuitem' | 'select-trigger' | 'combobox' | 'icon-button' | 'button' | 'generic-container';
...
export declare const BROAD_SELECTOR_MATCH_LIMIT = 50;
export declare const BROAD_SELECTOR_SCORE_LIMIT = 15;
export declare const NON_RENDERED_TAGS: Set<string>;
export declare const DOM_ORDER_TIEBREAKER_REASON = "resolved_by_dom_order_tiebreaker";
export declare function assertionNeedsDomContext(assertion: CodegenAssertion): boolean;
export declare function attachAssertionOutcomeSelections(session: CodegenSession, snapshotCache: SnapshotCache): void;

...
exports.validateScopedSelectorSpec = exports.validateSelectorSpec = exports.validateTextCandidate = exports.validateCSSCandidate = exports.isVisibleElement = void 0;
exports.hasSameAirTargetNodeId = hasSameAirTargetNodeId;
exports.shouldKeepOriginal = shouldKeepOriginal;
exports.matchesIntent = matchesIntent;
exports.generateCandidates = generateCandidates;
exports.complexityPenalty = complexityPenalty;
exports.volatilityPenalty = volatilityPenalty;
exports.scoreCandidate = scoreCandidate;
exports.normalizeLlmSelector = normalizeLlmSelector;
exports.normalizeLlmSuggestions = normalizeLlmSuggestions;
exports.normalizeLlmRetrySuggestions = normalizeLlmRetrySuggestions;
exports.resolveSelectorsForSession = resolveSelectorsForSession;
const types_1 = require("./resolver/types");
const text_matching_1 = require("./resolver/text-matching");
const element_ranking_1 = require("./resolver/element-ranking");
const visibility_1 = require("./resolver/visibility");
const selector_spec_validator_1 = require("./resolver/selector-spec-validator");
const structured_selector_utils_1 = require("./resolver/structured-selector-utils");
const excerpt_builder_1 = require("./resolver/excerpt-builder");
const selector_spec_1 = require("./selector-spec");
const selector_spec_2 = require("./selector-spec");
const selector_evaluation_1 = require("./selector-evaluation");
const bounded_field_1 = require("./resolver/bounded-field");
var visibility_2 = require("./resolver/visibility");
Object.defineProperty(exports, "isVisibleElement", { enumerable: true, get: function () { return visibility_2.isVisibleElement; } });
Object.defineProperty(exports, "validateCSSCandidate", { enumerable: true, get: function () { return visibility_2.validateCSSCandidate; } });
Object.defineProperty(exports, "validateTextCandidate", { enumerable: true, get: function () { return visibility_2.validateTextCandidate; } });
var selector_spec_validator_2 = require("./resolver/selector-spec-validator");
Object.defineProperty(exports, "validateSelectorSpec", { enumerable: true, get: function () { return selector_spec_validator_2.validateSelectorSpec; } });
Object.defineProperty(exports, "validateScopedSelectorSpec", { enumerable: true, get: function () { return selector_spec_validator_2.validateScopedSelectorSpec; } });
...
function resolveConfig(config) {
    const llmMaxCandidatesPerStep = Math.max(1, config?.llmMaxCandidatesPerStep ??
...
function evaluateClassTokenEntropy(classToken, step, candidate) {
    ...
    const calendarScopedClass = candidate.source === 'parent-scope' &&
        ((0, element_ranking_1.looksDateValue)(step.value) || step.action === 'input') &&
...
function computeClassEntropy(step, candidate, validation, snapshot) {
    ...
    const element = validation.resolvedElement ??
        (snapshot ? (0, element_ranking_1.getCandidateElement)(snapshot, candidate.selector) : null);
...
function computeIdEntropy(step, candidate, validation, snapshot) {
    ...
    const element = validation.resolvedElement ??
        (snapshot ? (0, element_ranking_1.getCandidateElement)(snapshot, candidate.selector) : null);
    const candidateId = (element?.getAttribute('id') ||
        element?.id ||
...
const CAPTURED_DIRECT_PROMOTION_FAMILY_PRIORITY = {
    'test-id': 1,
    id: 2,
    name: 3,
    href: 4,
...
const BLOCKED_CAPTURED_DIRECT_PROMOTION_WARNING_CODES = new Set([
    'multiple-matches',
...
const HIGH_TRUST_DIRECT_CATEGORIES = new Set([
    'testid',
    'id',
    'name',
    'href',
...
const CURRENT_WINNER_WEAK_WARNING_CODES = new Set([
...
function extractCapturedDirectCandidateValue(candidate) {
    switch (candidate.family) {
        case 'test-id':
            return (0, text_matching_1.extractAttributeValue)(candidate.selector, 'data-testid')
                ?? (0, text_matching_1.extractAttributeValue)(candidate.selector, 'data-cy')
                ?? (0, text_matching_1.extractAttributeValue)(candidate.selector, 'data-qa');
        case 'id':
...
function isCurrentWinnerHighTrust(draft) {
    ...
    if (!HIGH_TRUST_DIRECT_CATEGORIES.has(evaluation.category))
        return false;
    if (evaluation.proof.proofLevel !== 'snapshot_validated' &&
...
function evaluateCapturedDirectPromotionCandidate(step, snapshot, captured) {
    ...
    if (captured.family === 'id') {
        ...
        const idEntropy = computeIdEntropy(step, rawCandidate, {
            totalMatchCount: 1,
            visibleMatchCount: 1,
    ...
    if (!snapshot) {
        const pristineNoSnapshot = captured.strength === 'strong' &&
...
function maybeApplyCapturedCandidateDirectPromotion(draft, config) {
    ...
    const evaluatedCandidates = capturedCandidates
        .filter((candidate) => isAllowlistedCapturedDirectFamily(candidate.family))
        .map(candidate => ({
        captured: candidate,
...
function deriveSafeTriggerChildSelector(target, triggerSelector, container) {
    ...
    if (tagName === 'select')
        return 'select';
    const stableClass = (0, text_matching_1.findStableClassFromAttributes)(target.getAttribute('class') || undefined);
...
function buildLabelContextCandidates(step, snapshot, snapshotSelection) {
    ...
    let labels = [];
    try {
        labels = Array.from(snapshot.querySelectorAll('label')).filter(label => normalizeLabelContextText(label.textContent || '') === normalizedFieldLabel);
    }
    catch {
        labels = [];
    }
    ...
    const cleanParentSelector = matchingVisibleContainers.length === 1 &&
...
function buildTriggerContextCandidates(step, snapshot, snapshotSelection) {
    ...
    if (!cleanChildSelector)
        return [];
    const renderStatus = cleanParentSelector
...
function validateCandidate(selectorOrCandidate, snapshot, step) {
    if (typeof selectorOrCandidate === 'string') {
        ...
        return (0, text_matching_1.isTextSelector)(selector)
            ? (0, visibility_1.validateTextCandidate)(selector, snapshot, step)
            : (0, visibility_1.validateCSSCandidate)(selector, snapshot, step);
    }
...
function shouldKeepOriginal(step, snapshot, config, snapshotSelection) {
    ...
    const effectiveMinScore = (0, text_matching_1.isTextSelector)(step.selector)
...
function generateCandidates(step, snapshot, snapshotSelection) {
    ...
    const scopedBase = parentSelector && (0, text_matching_1.isLikelyCssSelector)(parentSelector)
        ? parentSelector
...
function extractOriginalMeaningfulText(step) {
    const text = step.fingerprint?.textExcerpt
        || ((0, text_matching_1.isTextSelector)(step.selector) ? (0, text_matching_1.extractTextExcerpt)(step) : null);
...
function textSignalsAlign(originalSignals, candidateSignals) {
    ...
    for (const original of normalizedOriginals) {
        ...
        for (const candidate of normalizedCandidates) {
            ...
            if (compactCandidate === compactOriginal ||
...
function collectCandidatePrimarySemanticSignals(element, snapshot) {
    return (0, text_matching_1.getElementSemanticTextSignals)(element, snapshot)
...
function collectCompatibilityEvidence(step, element, snapshot) {
    ...
    if (primaryOriginalSignals.length > 0 &&
...
function buildPreferredEquivalentRenderings(params) {
    ...
    if (params.selectorSpec.engine === 'scoped' ||
    ...
    const hasAlternateTestIdAttr = /\[data-cy=(?:"[^"]+"|'[^']+')\]/i.test(selector) ||
    ...
    if (params.category === 'placeholder' &&
        exactPlaceholder &&
    ...
    if (params.category === 'text' &&
        ...
        const matchedText = params.resolvedElement
            ? (0, text_matching_1.textFromElement)(params.resolvedElement)?.trim()
            : '';
        if (matchedText) {
            addRendering({
                engine: 'text',
...
function buildCandidateSelectorEvaluation(params) {
    const category = params.candidate.categoryOverride
        ?? (0, selector_evaluation_1.classifySelectorCategory)(params.candidate.selector, params.source ?? params.candidate.source);
    const proofLevel = params.proofLevel ?? (params.candidate.engine === 'bounded-field' &&
    ...
    const selectorSpec = params.candidate.engine === 'bounded-field' && params.candidate.boundedField
        ? (0, selector_spec_2.buildBoundedFieldSelectorSpec)({
    ...
    const semanticScore = Math.max(0, Math.min(1, params.semanticScore ??
        Math.max(params.validation.confidenceScore ?? 0, params.validation.effectiveMatchCount === 1 ? 0.5 : 0.2)));
...
function buildResolutionSelectorEvaluation(params) {
    ...
    const category = params.selectorSpec.engine === 'label-context' || params.selectorSpec.engine === 'trigger-context'
        ...
        : params.selectorSpec.engine === 'bounded-field'
            ? 'bounded-field'
            : (0, selector_evaluation_1.classifySelectorCategory)(params.selectorSpec.selector, params.selectorSpec.source);
    ...
    const preferredRenderings = params.validation
        ...
        : [];
    const proofScore = params.selectorSpec.proofLevel === 'recorded'
        ? 0.6
...
function resolveTriggerContextForStep(step, snapshot, snapshotSelection) {
    ...
    const warningCodes = new Set();
    const blockedReason = triggerMatches.matches.length > 1
    ...
    const isRecordedValid = (boundedField && boundedField.source === 'recorded-bounded-field') ||
        (candidate.source === 'interceptor' && candidate.triggerContext) ||
        (candidate.source === 'trigger-context');
    if ((validation.reason !== 'unique-visible' && !isRecordedValid) ||
        ...
        return {
            triggerContextLabel: labelText,
            triggerContextRenderStatus: 'blocked-unsafe-render',
            triggerContextRenderReason: validation.reason === 'non-unique'
...
function deriveDeterministicResolution(step, snapshot, config, ctx, snapshotSource, snapshotSelection, evaluatedCandidates) {
    ...
    if (snapshotSelection?.temporalClass !== 'outcome_state' &&
    ...
    const lowScoreFallbackCandidates = semanticallySafeCandidates
        ...
        .filter(candidate => candidate.candidate.selector !== step.selector &&
    ...
    if (!winner) {
        ...
        const boundedFieldRejectReason = (0, bounded_field_1.isWeakBoundedFieldInputCandidate)(step)
            ? (0, bounded_field_1.diagnoseBoundedFieldInputCandidate)({ step, snapshot, snapshotSelection }) ?? null
            : null;
...
function validateLlmCandidateSelector(draft, selector, config) {
    ...
    const match = !rejectReason && validation
        ? validation.resolvedElement ??
            (0, element_ranking_1.getCandidateElement)(draft.snapshot, selector) ??
            ((0, text_matching_1.isLikelyCssSelector)(selector) ? draft.snapshot.querySelector(selector) : null)
        : null;
...
function isLlmSelectorTooComplex(selector) {
    ...
    if (nthHeavyCount >= 2)
        return true;
    const depthCount = (selector.match(/\s+>\s+/g) ?? []).length +
    ...
    if (llmEnabled && llmTargets.length > 0 && llmFallbackProvider) {
        ...
        try {
            ...
            for (const target of llmTargets) {
                ...
                const retryEligible = !target.metadata.llmAccepted &&
            ...
            if (retryEligibleTargets.length > 0) {
                const retryRequest = {
                    ...
                    steps: retryEligibleTargets.map(target => ({
                        ...
                        snapshotExcerpt: excerptByStep.get(target.step.step)?.excerpt ??
                            (0, excerpt_builder_1.serializeSnapshotExcerpt)(target.snapshot, target.step, resolvedConfig.maxSnapshotExcerptChars).excerpt,

...
exports.SNAPSHOT_SKIP_REASONS = exports.SNAPSHOT_SELECTION_REASONS = void 0;
exports.selectSnapshotForAction = selectSnapshotForAction;
exports.selectSnapshotForOutcome = selectSnapshotForOutcome;
exports.selectSnapshotForStep = selectSnapshotForStep;
const visibility_1 = require("./resolver/visibility");
exports.SNAPSHOT_SELECTION_REASONS = {
    ...
    const maybeStaleForSameState = () => {
        if (typeof step.timestamp === 'number' &&
            ...
            return {
                ...
                annotationReason: exports.SNAPSHOT_SKIP_REASONS.IC_STALE,
            };
        }
        return {};
    };
    if (step.tabId && handle.tabId && handle.tabId !== step.tabId) {
        return { skipReason: exports.SNAPSHOT_SKIP_REASONS.TAB_MISMATCH, confidenceScore: 0 };
    }
    if (mode === 'action') {
        if (step.tabId && !handle.tabId) {
            return { confidenceScore: Math.min(baseConfidence, 0.5) };
        }
        if (step.controlSignature &&
            handle.controlSignature &&
            handle.controlSignature !== step.controlSignature) {
            return { skipReason: exports.SNAPSHOT_SKIP_REASONS.INCOMPATIBLE_STATE, confidenceScore: 0 };
        }
        const nextBoundary = findNextHardBoundaryForAction(step, inventory);
        if (nextBoundary &&
            typeof handle.timestamp === 'number' &&
            handle.timestamp > nextBoundary.timestamp + toleranceMs) {
            return { skipReason: exports.SNAPSHOT_SKIP_REASONS.STATE_BOUNDARY_CROSSED, confidenceScore: 0 };
        }
...
function tryFingerprintPresence(step, snapshot) {
...
function createUnavailableResult(evaluatedCandidates) {
    return {
        ...
        provenance: {
            ...
            reason: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_UNAVAILABLE_NO_SNAPSHOT_FOUND,
            confidenceScore: 0,
        },
        evaluatedCandidates,
    };
...
function buildActionCandidates(step, inventory) {
    ...
    if (!step.eventId) {
        ...
        candidates.push({
            ...
            reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_EVENT_LOCAL_PAGE_STATE_BY_EVENT_ID,
            skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.EVENT_LOCAL_UNAVAILABLE_MISSING_EVENT_ID,
            confidenceScore: 0,
        });
        candidates.push({
            source: 'event-local-pageSnapshot',
            temporalClass: 'action_local',
            reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_EVENT_LOCAL_PAGE_SNAPSHOT_BY_EVENT_ID,
            skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.EVENT_LOCAL_UNAVAILABLE_MISSING_EVENT_ID,
            confidenceScore: 0,
        });
    }
        ...
        candidates.push({
            ...
            temporalClass: 'action_local',
            reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_EVENT_LOCAL_PAGE_STATE_BY_EVENT_ID,
            skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.EVENT_LOCAL_PAGE_STATE_MISSING,
        ...
        candidates.push({
            ...
            temporalClass: 'action_local',
            reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_EVENT_LOCAL_PAGE_SNAPSHOT_BY_EVENT_ID,
            skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.EVENT_LOCAL_PAGE_SNAPSHOT_MISSING,
    ...
    candidates.push({
        ...
        reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_SOURCE_NODE_SNAPSHOT_AFTER_EVENT_LOCAL_MISSING,
        skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.SOURCE_NODE_SNAPSHOT_MISSING,
    ...
    if (!step.normalizedUrl) {
        candidates.push({
            ...
            reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_IC_EXACT_FOR_ACTION_FALLBACK,
            skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.NORMALIZED_URL_MISSING,
        });
    }
        ...
        candidates.push({
            ...
            temporalClass: 'post_action',
            reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_IC_EXACT_FOR_ACTION_FALLBACK,
            skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.EXACT_IC_SKIPPED_MISSING_CONTROL_SIGNATURE,
        });
    }
    else {
        ...
        candidates.push({
            ...
            temporalClass: 'post_action',
            reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_IC_EXACT_FOR_ACTION_FALLBACK,
            skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.INTERACTION_CONTEXT_EXACT_MISSING,
    ...
    candidates.push({
        source: 'interaction-context-stable-by-url',
        temporalClass: 'post_action',
        reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_IC_STABLE_BY_URL_FOR_ACTION_FALLBACK,
        skipReasonIfMissing: step.normalizedUrl
            ? exports.SNAPSHOT_SKIP_REASONS.INTERACTION_CONTEXT_STABLE_BY_URL_MISSING
            : exports.SNAPSHOT_SKIP_REASONS.NORMALIZED_URL_MISSING,
    ...
    candidates.push({
        ...
        temporalClass: 'unknown',
        reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_URL_EVENT_FALLBACK_LAST_RESORT,
        skipReasonIfMissing: step.normalizedUrl
            ? exports.SNAPSHOT_SKIP_REASONS.URL_EVENT_FALLBACK_MISSING
            : exports.SNAPSHOT_SKIP_REASONS.NORMALIZED_URL_MISSING,
...
function buildOutcomeCandidates(step, inventory) {
    ...
    if (step.normalizedUrl && step.controlSignature) {
        ...
        candidates.push({
            ...
            reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_IC_EXACT_STABLE_FOR_OUTCOME_STATE,
            skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.INTERACTION_CONTEXT_EXACT_MISSING,
        ...
        candidates.push({
            source: 'interaction-context-exact',
            temporalClass: 'outcome_state',
            reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_IC_EXACT_FOR_OUTCOME_STATE,
            skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.INTERACTION_CONTEXT_EXACT_MISSING,
            handle: inventory.interactionContextExactAny.get(exactKey),
        });
    }
        ...
        candidates.push({
            ...
            skipReasonIfMissing: !step.normalizedUrl
                ? exports.SNAPSHOT_SKIP_REASONS.NORMALIZED_URL_MISSING
                : exports.SNAPSHOT_SKIP_REASONS.EXACT_IC_SKIPPED_MISSING_CONTROL_SIGNATURE,
        });
        candidates.push({
            source: 'interaction-context-exact',
            temporalClass: 'outcome_state',
    ...
    candidates.push({
        ...
        temporalClass: 'outcome_state',
        reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_OUTCOME_EVENT_SNAPSHOT,
        skipReasonIfMissing: step.traceId
            ? exports.SNAPSHOT_SKIP_REASONS.OUTCOME_EVENT_SNAPSHOT_MISSING
            : exports.SNAPSHOT_SKIP_REASONS.EVENT_LOCAL_UNAVAILABLE_MISSING_EVENT_ID,
    ...
    candidates.push({
        ...
        temporalClass: 'outcome_state',
        reasonIfSelected: exports.SNAPSHOT_SELECTION_REASONS.SELECTED_DESTINATION_NODE_SNAPSHOT_AFTER_OUTCOME_MISSING,
        skipReasonIfMissing: exports.SNAPSHOT_SKIP_REASONS.DESTINATION_NODE_SNAPSHOT_MISSING,
...
function evaluateTargetPresence(step, candidate, snapshot, mode) {
    ...
    if (!targetPresent && probableClosedShadow) {
        return {
            targetPresent: false,
            skipReason: exports.SNAPSHOT_SKIP_REASONS.CLOSED_SHADOW_DOM_UNOBSERVABLE,
            shadowDegraded: true,
    ...
    if (!targetPresent && step.nestedContext?.isShadowDom) {
        return {
            targetPresent: false,
            skipReason: exports.SNAPSHOT_SKIP_REASONS.SHADOW_DOM_NOT_SERIALIZED,
            shadowDegraded: true,
    ...
    if (!targetPresent) {
        return {
            targetPresent: false,
            skipReason: exports.SNAPSHOT_SKIP_REASONS.TARGET_MISSING_IN_SNAPSHOT,
...
function selectFromCandidates(step, candidates, inventory, mode) {
    ...
    for (const candidate of candidates) {
        if (selectedCandidate && !weakLabelFallback) {
            evaluatedCandidates.push(createTraceEntry(candidate, {
                skipReason: exports.SNAPSHOT_SKIP_REASONS.HIGHER_PRIORITY_CANDIDATE_ALREADY_SELECTED,
        ...
        if (!snapshot) {
            evaluatedCandidates.push(createTraceEntry(candidate, {
                skipReason: exports.SNAPSHOT_SKIP_REASONS.SNAPSHOT_PARSE_FAILED,
                confidenceScore: deriveBaseConfidence(candidate, mode),
            }));
            continue;
        }
        ...
        const icTabUnknown = isInteractionContextSource(candidate.source) &&
            !!step.tabId &&
        ...
        if (mode === 'outcome') {
            evaluatedCandidates.push(createTraceEntry(candidate, {
                ...
                skipReason: icStateEvaluation.annotationReason
                    ?? (icTabUnknown ? exports.SNAPSHOT_SKIP_REASONS.IC_TAB_UNKNOWN : undefined),
            }));
        ...
        const shouldReplace = !selectedCandidate
        ...
        if (shouldReplace) {
            if (selectedCandidate) {
                const previousIndex = evaluatedCandidates.findIndex(entry => entry.selected
    ...
    const provenance = {
        ...
        confidenceScore: selectedConfidence,
        snapshotTargetEvidence: evaluatedCandidates.find(entry => entry.selected &&
            entry.source === selectedCandidate?.source &&
            entry.temporalClass === selectedCandidate?.temporalClass)?.targetPresent,
        snapshotTargetEvidenceReason: evaluatedCandidates.find(entry => entry.selected &&
            entry.source === selectedCandidate?.source &&
            entry.temporalClass === selectedCandidate?.temporalClass)?.snapshotTargetEvidenceReason ?? null,

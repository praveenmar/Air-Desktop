...
exports.scanPageAnchors = scanPageAnchors;
exports.scanCompositeAnchors = scanCompositeAnchors;
exports.scanCompositeAnchorsDetailed = scanCompositeAnchorsDetailed;
...
const FORM_CONTROL_SELECTOR = 'input, select, textarea, button, [role="button"], [data-testid], [name]';
const ACTION_CONTROL_SELECTOR = 'button, [role="button"], input[type="submit"], input[type="button"], input[type="reset"], a[href]';
const MENU_CONTROL_SELECTOR = '[role="menuitem"], [role="option"], option, button, [role="button"], a[href], li';
function nowMs() {
...
function isStableTextCandidate(value) {
    ...
    if (!normalized)
        return false;
    if (normalized.length < 2 || normalized.length > NON_VOLATILE_TEXT_MAX_LENGTH)
        return false;
    if (isLikelyVolatile(normalized))
        return false;
    return true;
}
...
function stableAttributeToken(el, tagName) {
    ...
    const testId = normalizeAnchor(safeGetAttribute(el, "data-testid") ||
...
function deriveScopeIdentity(container) {
    ...
    const labelSource = safeGetAttribute(container, "aria-label") ||
    ...
    const scopeToken = stableAttributeToken(container, scopeTag || "scope") ||
...
function firstStableText(elements, prefix) {
...
function getStableRowIdentifier(row) {
    ...
    const firstCellToken = stableAttributeToken(firstCell, firstCell.tagName.toLowerCase()) ||
...
function selectTopAnchors(candidates) {
    ...
    for (const [kind, list] of byKind.entries()) {
        const sorted = [...list].sort((left, right) => right.sortScore - left.sortScore ||
...
function chooseStableControlToken(el) {
    return (stableAttributeToken(el) ||
...
function generateFormAnchor(form) {
    ...
    const fieldTokens = controls
        ...
        const nameToken = tokenFromParts(`${tag}.name`, normalizeAnchor(safeGetAttribute(control, "name"))) ||
    ...
    const formActionToken = formAction && !isLikelyVolatile(formAction)
        ? tokenFromParts("form.action", formAction.toLowerCase())
        : null;
    const headingToken = tokenFromParts("form.label", extractHeadingText(form));
    return buildCompositeAnchor("form_cluster", form, [formActionToken, headingToken, ...fieldTokens], 0.95);
}
...
function generateTableRowAnchors(table) {
    ...
    for (const row of rows) {
        ...
        if (actionTokens.length === 0)
            continue;
        const anchor = buildCompositeAnchor("table_row", row, [...headerTokens, rowIdentifier, ...actionTokens], 0.96);
        if (anchor)
            anchors.push(anchor);
    }
    return anchors;
...
function generateDialogAnchor(dialog) {
    const titleToken = tokenFromParts("dialog.title", extractHeadingText(dialog)) ||
    ...
    const actionTokens = collectControls(dialog, ACTION_CONTROL_SELECTOR, 8, 5)
        ...
        .filter((token) => !!token)
        .slice(0, 6);
    return buildCompositeAnchor("dialog_actions", dialog, [titleToken, ...actionTokens], 0.93);
}
function generateMenuAnchor(container) {
    const labelToken = tokenFromParts("menu.label", normalizeAnchor(safeGetAttribute(container, "aria-label"))) ||
    ...
    const optionTokens = collectControls(container, MENU_CONTROL_SELECTOR, 10, 4)
        ...
        .filter((token) => !!token)
        .slice(0, 8);
    return buildCompositeAnchor("menu_group", container, [labelToken, ...optionTokens], 0.9);
}
function generateContainerAnchor(container) {
    const controlTokens = collectControls(container, ACTION_CONTROL_SELECTOR, 8, 4)
        ...
        .filter((token) => !!token)
        .slice(0, 6);
    const labelToken = tokenFromParts("container.label", normalizeAnchor(safeGetAttribute(container, "aria-label"))) ||
        tokenFromParts("container.heading", extractHeadingText(container));
    return buildCompositeAnchor("container_controls", container, [labelToken, ...controlTokens], 0.82);
}
...
function scanPageAnchors(root = globalThis.document, locationPath = typeof window !== "undefined" ? window.location.pathname : "/") {
    ...
    const elements = root.querySelectorAll('input, button, select, textarea, form, h1, h2, h3, [role="button"]');
    for (const rawEl of elements) {
        if (!rawEl || typeof rawEl.tagName !== "string")
            continue;
        const tag = rawEl.tagName.toUpperCase();
        const styleAttr = typeof rawEl.getAttribute === "function" ? rawEl.getAttribute("style") || "" : "";
        // Visibility checks
        if (tag === "INPUT" && normalize(rawEl.getAttribute?.("type") || "").toLowerCase() === "hidden")
            continue;
        ...
        try {
            if (rawEl.hasAttribute("data-testid")) {
                ...
                if (inputType.length > 1)
                    anchors.push(`${tag}:type=${inputType}`);
            }
        }
        catch (e) {
...
function scanCompositeAnchorsDetailed(root = globalThis.document, _locationPath = typeof window !== "undefined" ? window.location.pathname : "/") {
    ...
    for (const form of forms)
        inspectContainer("form_cluster", form, generateFormAnchor);
    formScanMs = nowMs() - scanStartedAt;
    scanStartedAt = nowMs();
    const dialogs = scanContainers(root, 'dialog, [role="dialog"]', MAX_CONTAINERS_PER_KIND);
    for (const dialog of dialogs)
        inspectContainer("dialog_actions", dialog, generateDialogAnchor);
    dialogScanMs = nowMs() - scanStartedAt;
    scanStartedAt = nowMs();
    const tables = scanContainers(root, "table", MAX_CONTAINERS_PER_KIND);
    for (const table of tables)
        inspectContainer("table_row", table, generateTableRowAnchors);
    tableScanMs = nowMs() - scanStartedAt;
    scanStartedAt = nowMs();
    const menus = scanContainers(root, '[role="menu"], [role="listbox"], [role="list"], nav, [role="group"]', MAX_CONTAINERS_PER_KIND);
    for (const menu of menus)
        inspectContainer("menu_group", menu, generateMenuAnchor);
    menuScanMs = nowMs() - scanStartedAt;
    scanStartedAt = nowMs();
    const containers = scanContainers(root, 'section, [role="region"], [role="group"], [role="toolbar"], [role="navigation"], main, aside', MAX_CONTAINERS_PER_KIND);
    for (const container of containers)
        inspectContainer("container_controls", container, generateContainerAnchor);
    containerScanMs = nowMs() - scanStartedAt;
    const selected = selectTopAnchors(candidates.filter((candidate, index, arr) => arr.findIndex((other) => other.descriptor === candidate.descriptor) === index));

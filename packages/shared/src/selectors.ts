...
export function rankForPriority(priority: string): number {
...
export function findStableClass(element: Element): string | null {
...
export function generateOptimalSelector(
  rawElement: Element,
  options: SelectorOptions = {}
): SelectorResult {
  const element = rawElement as SelectorElement;
  const tagName = element.tagName.toLowerCase();
  const maxTextLength =
    typeof options.maxTextLength === "number" ? options.maxTextLength : 50;
